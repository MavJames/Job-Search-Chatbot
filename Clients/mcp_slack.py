...
# Suppress the confusing deprecation warning from LangGraph
...
CAPABILITIES_PATH = (
    Path(__file__).resolve().parent / "../Servers/server_capabilities.json"
)
...
class LocalSessionManager:
    ...
    def get_resume_info(self, user_id: str) -> dict:
        ...
        return self.user_resumes.get(
            user_id, {"resume_text": None, "resume_path": None}
        )
...
def extract_text_from_pdf(pdf_content) -> str | None:
...
def extract_text_from_docx(docx_content) -> str | None:
...
def extract_resume_text(file_content, filename: str) -> str | None:
...
def handle_file_upload(event, client, say):
    ...
    try:
        ...
        if resume_text:
            # Save it locally
            file_path = session_manager.save_resume_file(
            ...
            say(
                "✅ Resume uploaded successfully! I'll use this info to help with your job search."
            )
        else:
            say(
                "Could not extract text from resume. Please make sure it's a valid PDF or Word document."
            )
